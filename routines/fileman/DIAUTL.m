DIAUTL	;GFT/GFT - UTILITIES TO TURN ON AND TO ANALYZE FILEMAN AUDITS ;24MAY2012
...
DISP(DDB)	;DISPLAY DD CHANGES FROM ^DDA SINCE DATE 'DDB'
	N DIA,FR,BY,TO,DHD,DDHD,DIC,L,POP,DDIO,DIOEND,DDTOUT
	D ^%ZIS Q:POP  S DDIO=ION
	F DIA=0:0 S DIA=$O(^DDA(DIA)) Q:'DIA  S FR=$O(^DDA(DIA,"D",DDB)) D:FR  Q:$D(DDTOUT)  W @IOF
	.S DIC="^DDA("_DIA_",",BY="-(#.03)@",TO=DT+1,FLDS="[DIAUTL]",L=0
	.S DHD="W ?0 D DDHD^DIAUTL",IOP=DDIO,DIOEND="S:$G(DIOO1) DDTOUT=1"
	.D EN1^DIP
	Q
DDHD	S DDHD="DATA DICTIONARY CHANGES, "_$P($G(^DIC(DIA,0)),U)_" FILE(#"_DIA_")" S:DDB DDHD=DDHD_"    SINCE "_$$DATE^DIUTL(DDB)
	W DDHD,?IOM-9," Page ",$G(DC),!
